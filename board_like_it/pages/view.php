...
<body>
    ...
    <?php
        $num = $_GET['num'];
        $sql ="SELECT subject, content, date, hit, id FROM board2 WHERE num = $num";
        $res = mysqli_query($conn,$sql);
        $rows = mysqli_fetch_array($res);
        $content = str_replace("\n","<br>",$rows['content']);  
        $content = str_replace("","&nbsp;",$content);
        $hit="UPDATE board2 SET hit=hit+1 WHERE num=$num";
...
<div class="riple_wrap">
    ...
    <?php $sql = "SELECT * FROM riple WHERE parent='$num'order by num desc";
    ...
    <form class="riple_view_wrap" method="post">
        ...
        <?php
            ...
            ?><button class="riple_del_bt" onclick="return confirm('삭제하시겠습니까?')"><a href="../actions/riple_delete.php?num=<?=$riple_num?>">삭제</a></button>
        <?php    }
        ?>
